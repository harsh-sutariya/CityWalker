...
import yaml
import os
from pl_modules.citywalk_pretrain_datamodule import CityWalkPretrainDataModule
from pl_modules.vjepa_module import VJEPAModule
...
def main():
    ...
    if cfg.data.type == 'citywalk':
        datamodule = CityWalkPretrainDataModule(cfg)
    else:
        raise ValueError(f"Invalid dataset: {cfg.data.dataset}")

    # Initialize the model
    model = VJEPAModule(cfg)
